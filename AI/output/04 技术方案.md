...
| 实测 | 本机MySQL、Redis、Milvus和模型、地图、邮件接入参数已落在启动模块的本地YAML；未在方案中输出敏感值 | SRC-003 | 本地调试不依赖进程环境变量；发布期可由企业配置中心覆盖同名配置项 |
...
| 已确认 | 验证码统一登录、首次自动建号、密码设置/重置撤销旧会话 | 02产品方案 | 登录码不以账号存在性分支；密码为可选凭据 |
...
| hello-travel-util | 内部模块共享的无状态技术工具 | 当前只依赖Jackson；只允许application/adaptor/infrastructure按需使用，不进入client/model/domain契约 |
...
| hello-travel-application | Command/Result/assembler/用例/OutAdaptor/工作流；domain/model/common/util | Spring组件/短事务边界、LangGraph4j core用于编排；不依赖具体SDK、PO或adaptor实现 |
...
| hello-travel-adaptor | input Controller/SSE/scheduler，output外部读写转换；application/client/model/common/util | LangChain4j OpenAI兼容客户端、Milvus SDK、高德HTTP、SMTP、PDF解析，第三方对象私有 |
| hello-travel-start | Application、扫描/装配、数据库迁移、Logback/安全/模型配置 | 装配7个业务运行模块，util经实际消费者进入运行时；无业务流程，配置可部署 |
...
**实施版本基线（TECH-002）**：已完成真实源码构建、默认版本安装及独立消费检查；具体证据见05。根集中固定Java17、Spring Boot3.5.16、MyBatis-Plus3.5.17、LangChain4j1.19.0（Spring Boot starter 1.19.0-beta29）、LangGraph4j core1.8.27、Milvus Java SDK3.0.5。前端核验registry：React19.3.0、Ant Design6.6.4、Vite8.3.0、TypeScript7.0.2；Node24.18.0满足已核验Vite的Node版本条件。实际前端版本由package-lock.json锁定；编译、类型与装配检查通过不代表外部服务已经联调。Flyway固定11.20.3，并排除PDFBox传递的commons-logging，由Spring JCL提供日志桥。

Spring Boot3.5的Java要求见[官方系统要求](https://docs.spring.io/spring-boot/3.5/system-requirements.html)；LangChain4j采用[官方 Spring Boot starter](https://docs.langchain4j.dev/tutorials/spring-boot-integration/)，以`langchain4j.open-ai`的YAML配置自动装配聊天、流式聊天和嵌入模型，并通过[百炼OpenAI兼容接口](https://help.aliyun.com/zh/model-studio/compatibility-of-openai-with-dashscope)接入。LangGraph4j core作为[StateGraph编排库](https://github.com/langgraph4j/langgraph4j)，不引入未使用的langchain集成模块。Milvus3.0.x与SDK基线参照[官方兼容表](https://milvus.io/api-reference/java/v3.0.x/About.md)。
...
- 登录验证码消费、首次账号创建/读取和会话签发均在同一短事务；密码在外层按快照验证后，提交CAS必须核验同一credential版本，重置后不能复用旧密码校验结果；验证码成功消费与登录/密码设置或重置在同一事务提交，避免业务失败却先吞码。失败次数CAS递增且与成功消费互斥，过期/用途不符不能消费。验证码挑战同邮箱用途唯一有效槽。
...
| `password_hash` | `VARCHAR(255) NULL` | 可选的带算法和参数的加盐密码哈希；验证码首次登录账号为空，不存明文 |
...
| `id` | `BIGINT UNSIGNED NOT NULL AUTO_INCREMENT` | 内部主键，不直接作为前端数值ID |
| `public_id` | `CHAR(26) CHARACTER SET ascii COLLATE ascii_bin NOT NULL` | 对外ULID字符串标识 |
| `email_normalized` | `VARCHAR(254) COLLATE utf8mb4_0900_as_cs NOT NULL` | 目标邮箱，登录码签发时可尚无账号 |
| `purpose` | `VARCHAR(16) NOT NULL` | 登录或密码设置/重置用途 |
...
| `id` | `BIGINT UNSIGNED NOT NULL AUTO_INCREMENT` | 内部主键，不直接作为前端数值ID |
| `public_id` | `CHAR(26) CHARACTER SET ascii COLLATE ascii_bin NOT NULL` | 对外ULID字符串标识 |
| `user_id` | `BIGINT UNSIGNED NULL` | 可选账号归属；首次登录验证码可尚无账号 |
...
| `stage` | `VARCHAR(32) NOT NULL` | 模型调用阶段：`INTENT`、`DIALOGUE`、`PLAN_DRAFT`、`PLAN_REVISION`、`MEMORY_SUMMARY` 或 `MEMORY_EXTRACTION`；它与旅行意图路由分开，便于审计普通对话、规划和记忆调用 |
...
LoginApplication.login(command):
  verify password against credential-version snapshot, or validate LOGIN code proof
  ...
  shortTransactionWithBoundedRetry:
    for password login: require an existing ACTIVE account with configured password, then recheck credential-version/auth_epoch
    for code login: lookup the normalized email; if absent, create one ACTIVE email-verified account with password_hash NULL under the unique-email constraint
    CAS consume the purpose-bound unexpired LOGIN challenge in the same transaction as account creation/retrieval and session issuance
...
| ID | 契约 | 输入/输出与边界 |
|---|---|---|
| API-001 | POST /api/v1/auth/challenge | 邮箱＋`LOGIN`或`RESET_PASSWORD`用途；200 challengeId受理，异步真实投递；缺SMTP503；登录码不按账号是否存在分支 |
| API-002 | POST /api/v1/auth/login | email＋password，或 email＋LOGIN challenge/code；首次有效登录码原子创建账号；deviceCookie；返回access/sid/user，refresh HttpOnly |
| API-003 | POST /api/v1/auth/refresh；POST /logout | expectedSid＋CSRF/Origin；只轮换原sid或撤销当前，不自动接入新sid |
| API-004 | POST /api/v1/auth/reset | RESET_PASSWORD挑战/码＋新密码；为验证码账号设置首个密码或重置已有密码，撤销所有旧设备会话 |
...
默认配置契约全部由启动模块`application.yml`表达：`spring.datasource`、`spring.data.redis`、`spring.mail`、`langchain4j.open-ai`、`travel.knowledge`、`travel.amap`、`travel.security`以及上下文预算。适配器通过明确的配置项构造注入，不使用通用`Environment`查询。当前本地环境按需求使用明文YAML；企业发布期由受控配置服务覆盖同一配置键，不依赖服务器进程环境变量。缺关键配置报明确不可用或启动配置错误，不替换成demo secret/假成功。
...
| TASK-003 | F001～007/TECH-003 | auth domain/app/infra/adaptor；认证动作AppService→领域聚合/服务→仓储实现 | 001/002 | 真实账号/设备/session/challenge模型、首次登录原子建号、可选密码哈希、Redis频控、可配置SMTP，不用固定码 | 并发首次登录/码重放/设备内替换/旧页shared Cookie/密码设置与重置撤销 |
...
前端工程位于后端同级`../hello-travel-app`，有独立package.json/lock/README/忽略规则和构建命令。后端本地运行配置只在启动模块`application.yml`维护；后续发布由受控配置服务接管同名配置项。AI接口说明、数据库设计说明、知识资料和检查证据归`AI/output`；已执行的正式Flyway V1仍在start模块资源目录，17张业务表与迁移校验和不变。
...
SRC-023纠正实现对已有FMT-005/DDD约定的遗漏，不改变一期功能、17张MySQL表或Milvus专属集合。应用层协调权限、数据库事务、服务调用和证据；Entity承担自身初始化、归属/版本不变量和状态变化，Aggregate提供场景语义入口；DomainService协调仓储和聚合规则；Repository负责PO映射与CAS；Value表达完整凭据和有原文来源的事实提案。不得以容器名称替代行为封装。

重新推演的正常链路是对话归属→用户/助手消息对→任务接受→记忆/RAG/预算→模型证据→回复与持久同步事件。跨账号消息关联、未持久化消息、旧版本写入、密码重置后的认证代次、同浏览器会话替换、迟到消息覆盖、删除后全量历史替换、非法事实类别/幻觉摘录、重复任务领取均有明确拒绝或恢复职责；本轮相关单元反例通过，完整服务联调仍待后续阶段。

标题创建/重命名复用同一实体边界；会话工厂复制凭据且固定有效期；知识分块公开标识与vectorKey同源。MemoryFlow与KnowledgeIndexer按业务阶段拆分，保留原租约/代次、调用登记、外部计费结果未知不自动重放、事务回滚和删除生命周期。19处应用全字段初始化归入领域工厂。新门禁只验证编号、规模和有限结构，不能据此声称所有状态/故障/资源条件已证实。

后续需求迭代先阅读既有完整链路，再决定复用、重构、替换和删除；修改后检查全部受影响方法及上下游，清理失效注释与重复状态处理，再执行相应正常/失败反例。证据与未验证边界集中05和业务方法复核记录，避免继续向长流程堆叠分支。本轮仍以05编码交付为终点，无人天估算。


当前质量证据是本轮编码整改和针对性开发回归，不是独立CR、完整业务验收或生产认证。真实百炼/嵌入/高德/SMTP、多设备真实同步与整套故障演练未执行。既有迁移不改写，本轮未操作MySQL、Milvus或Redis数据。

## 9. 0.6映射边界、异常归属与配置可行性推演

SRC-025暴露上一轮实际转换职责漏检。Controller→InputAssembler→Application→Command/ApplicationAssembler→Domain或OutputAdaptor→Converter的映射边界现已实质落实，17组PO/聚合转换由专属PersistenceConverter承接。Assembler接收完整来源或明确用途输入，避免工作流和HTTP入口散落字段展开；删除ApiViews反射通用投影与标量view辅助。Domain不承担HTTP动作解析，应用与协议分别创建本层异常，下层已分类失败通过公共契约保留。

模型预算由ModelContextPolicy统一，配置键travel.model.context-window / output-reserve / safety-reserve / compression-input-limit默认131072/8192/4096/96000。应用窗口选择128K而非供应商最大窗口，使上下文压缩、延迟与成本边界可预测；配置非法在初始化拒绝。辅助输出最大2048且受配置output-reserve约束，回答输出使用配置预留；所有非嵌入模型输入在IO前估算容量并拒绝超预算。前端只展示服务端本轮预算，且明确该百分比包含回答与安全预留，缺失视为未知。KnowledgeIndexPolicy统一text-embedding-v4、1024和hello_travel_kb_v1_d1024；Domain通过EmbeddingProfileValue接受已选策略；MySQL元数据和Milvus向量职责不混淆。

反例推演及对应验证：未知auth路由不能触发设备签名；未可信历史角色不能升为SYSTEM；刷新Cookie不能替代原页面SID；非法向量或超预算模型输入不能发生IO；converter已分类错误不能丢失；修改上下文容量不能造成前端/压缩/SDK输出上限不一致；持久化映射不能丢失版本、归属与删除时间。对应35后端/4前端针对性回归通过，配置和映射证明不替代真实供应商联调与完整并发验收。详细链路及逐类清单见docs/02业务方法复核记录与05第9章。正式V1与一期功能不变，默认不估人天。

## 10. 0.7业务垂直领域与域内模式（当前实现）

此前把写模式误当全项目横向服务，TravelWriteDomainService/DomainWrites/TravelWriteApplicationAssembler/TravelRepositories汇总所有业务，违背业务内聚性，现已删除，不保留兼容总门面。业务边界沿现有auth、chat、memory、knowledge、sync划分；每个业务内部按实际需要选择写入、域内读、外部读、规则计算、纯计算。五个业务子域与五种处理模式是两个不同维度；当前五个写服务不是“每种模式一个服务”的映射，也不要求未来固定服务数量。

| 业务 | DomainService位置 | 域内仓储/聚合 | 应用写入/转换/读取 |
|---|---|---|---|
| 认证auth | domain/auth/service/AuthDomainService | UserAccount、Device、LoginSession、EmailChallenge、RefreshReceipt | 认证业务动作直接调用AuthDomainService；AuthDomainParamAssembler只转换明确领域参数 |
| 对话chat | domain/chat/service/ChatDomainService | Conversation、Message、ChatRun、ModelInvocation | 对话业务动作直接调用ChatDomainService；ChatDomainParamAssembler只转换明确领域参数 |
| 记忆memory | domain/memory/service/MemoryDomainService | MemorySummary、MemoryFact、MemoryFactSource | 记忆提取、压缩和隐私清理直接调用MemoryDomainService；MemoryDomainParamAssembler转换领域参数 |
| 知识库knowledge | domain/knowledge/service/KnowledgeDomainService | KnowledgeDocument、KnowledgeChunk、IndexJob | 上传、索引、重试和删除流程直接调用KnowledgeDomainService；KnowledgeDomainParamAssembler转换领域参数 |
| 同步sync | domain/sync/service/SyncDomainService | SyncEvent、OutboxEvent | 事件发布、发件箱投递和保留任务直接调用SyncDomainService；SyncDomainParamAssembler转换领域参数 |

各域Param归domain/<业务>/model/param，应用assembler和协作归application/<业务>；各领域服务构造器只接本域仓储。领域模型、仓储接口、持久化转换及MySQL正式迁移均不变。WriteDO仅为共享写入结果标记，不承载领域路由；ApplicationFailures.required仅负责失败结果中断事务，不调用仓储或分发业务。

领域服务名称表达业务职责，不用`Write`后缀重复用例模式；保存、删除等领域方法和Param保留具体动作语义。Application不再设置`*Writes`或`*WriteAppService`作为通用持久化中转，统一邮箱登录、密码设置/重置、会话、消息、记忆、知识索引与同步发布等业务流程通过各自`DomainParamAssembler`转换完整聚合后直接调用所属DomainService。Application读取也不通过`*Repositories`聚合；构造器依赖据此暴露真实职责规模。

普通读仍由本业务Application访问相应仓储并经assembler转换，外部读仍经对应OutAdaptor；不为形式凑服务。隐私清理显式调用memory和chat领域能力，后台保留策略显式调用auth和sync领域能力；跨域控制在应用层，不将其他域仓储注入DomainService。Transactions仍以认证账号版本控制提交序列，SyncEventPublisher只调用SyncDomainService，无循环装配。

可行性再推演：登录与事件提交、消息与同步、记忆及消息清除、索引及账号提交序列均保留同一事务与顺序；20个保留操作的规则方法体AST与原实现相同，17保存和3实际清理均有调用方。删除14个没有调用方的物理删除入口及Param，不改变页面逻辑删除、保留策略或二期功能。归属/版本、目标缺失、CAS、底层不可用、跨域提交失败与装配反例回归通过；真实MySQL并发/故障、多设备和供应商联调仍待后续既定阶段，不能以逻辑推演冒充运行验证。

当前代码294源文件、790方法/构造器、371多语句流程块，默认门禁0违规；主入口36处结构通过。39项后端、DDD根和独立快照各7项回归通过，前端本轮未改未重跑。规范1.13、快照1.11；本次仍止于05，无默认工时估算。

## 11. 0.8产品主线、技术端口与Agent装配（当前实现）

SRC-052进一步指出上一轮仍在application/adaptor一级按HTTP、model、file、mail等技术概念分包。当前改为认证、对话、知识库三条产品主线；同步、记忆与旅行规划是对话内部能力，邮件与安全归认证，文件、向量和嵌入归知识库，跨业务HTTP协议支撑收敛在`adaptor.common`。一级`adaptor/http`、`application/model`和`adaptor/model`均已删除。

原通用模型命令依赖`action + 多组可空字段`，把意图、摘要、事实提取、回答和嵌入组合成一个端口，新增能力容易继续增加分支。当前按照业务目的拆成`TravelIntentRecognitionAdaptor`、`TravelDialogueAdaptor`、`TravelPlanGenerationAdaptor`、`MemoryAgent`和`KnowledgeEmbeddingAgent`；提示规则和固定参数由所属assembler承载，Application不再识别模型厂商或动态动作。共享结果也按对话和知识嵌入分开，避免一个DO同时携带文本与向量两种互斥形态。

对话侧LangChain4j接口集中在`adaptor.chat.output.aiservice`并使用显式`@AiService`；start提供命名`ChatModel`和`StreamingChatModel`，回答接口返回Flux并由适配器累积完整草稿、限制长度、执行租约进度回调。知识侧start提供命名`EmbeddingModel`，索引与查询复用同一模型实例。无密钥时全部装配禁用模型，保证离线启动不触发外部调用。

没有直接把RAG改成自动ContentRetriever。当前候选向量返回后必须查询MySQL并核对账号、READY状态、删除状态、索引代次、文档标识和正文SHA；这是业务隔离与失效治理，不能由只负责相似度召回的基础检索器替代。将来可在保留相同后置治理的前提下包装ContentRetriever，但不能将其直接注入Agent绕过校验。

反例重新推演：高阶AiService不能损失原流式更新，因此补充StreamingChatModel与累积回调测试；历史实体不能依赖默认toString进入提示，因此由assembler明确投影角色和正文；模型客户端不能按请求重复创建，因此统一迁到start命名Bean；知识嵌入不能与对话窗口策略混用，因此端口、命令、结果和转换器分别归业务。根clean test的40项JUnit、离线装配及322类/819方法构造器/376流程块门禁通过。真实供应商断流、usage、Milvus召回质量与多设备体验仍待06及后续阶段验证。

## 12. 1.2输入适配器内部结构（当前修订）

每条业务输入边界将Controller直接放在`<业务>.input`，assembler放在`<业务>.input.assembler`。Controller承载HTTP入口、边界捕获和调用编排；Assembler承载Request/path/query与Command、Application Result与Response之间的协议转换。SSE入口同样属于对话业务input，不因传输方式另立业务维度。

认证、对话和知识库输入代码均已按用户当前包树迁移。项目`QUALITY-PACKAGE`在Java语法扫描阶段拒绝Controller离开业务input根包或Assembler离开input.assembler；当前构建证明Spring扫描、测试引用和HTTP映射未因包迁移失效。此项目修订尚未同步为公共DDD规范。

## 13. 1.0旅行对话三路编排与规划 Graph（当前实现）

### 13.1 调用链和职责

对话 HTTP 入口继续通过 `ChatApplication` 完成归属校验、用户消息、生成任务和 Outbox 的事务提交，随后由 `BackgroundJobDispatcher` 领取 `GENERATE` 事件并调用 `TravelApplication`。这条异步入口保留消息先持久化、断线恢复、租约栅栏和模型调用不占 HTTP 事务的生产语义；三路业务判断集中在 `TravelApplication`，没有散落到 Controller、后台派发器或模型实现中。

`TravelApplication`准备会话、短期/长期记忆与上下文预算，调用结构化意图端口后只做三个明确分支：`DIALOGUE`进入普通流式对话；`FACT_QUERY`先调用天气/路线与知识库证据，再进入普通流式对话；`PLANNING`进入 `TravelPlanningGraph`。未知意图只按已提取的日期、天数和事实标记保守兜底，意图端口失败直接受控失败，不以散落关键字静默猜测。

### 13.2 规划图及层间协作

当前图固定为 `validate_request → collect_destination_evidence → collect_realtime_facts → generate_draft → validate_draft → revise_draft（最多两次）→ render_plan`，需求缺失、不可修订或达到上限时进入 `clarify_requirements`。节点依据一期咨询规划边界从公开产品能力推导，完整依据见[旅行 AI 规划 Graph 设计](<docs/旅行AI规划Graph设计-SRC055.md>)。

LangGraph4j 只在 Application 的 `chat.travel.planning.graph`及节点包中出现；节点按业务需要调用知识检索、外部事实 OutAdaptor、结构化计划生成端口和 `TravelPlanDomainService`。LangChain4j `@AiService`及供应商结果转换位于对话 OutAdaptor，Application 契约和稳定中间模型不暴露框架类型。领域服务确定性校验日期、天数、人数、时间重叠、接驳、预算以及未核实事实标记；模型产生的是草稿，不能绕过领域规则形成最终计划。

### 13.3 可行性反思和实际修订

首轮测试把 `TravelConversationContext`直接放入 LangGraph 状态，框架在克隆状态时触发 `NotSerializableException`。若让所有领域、消息和模型对象实现 `Serializable`，会把 LangGraph 的技术限制扩散到稳定业务模型，也会增加将敏感上下文写入 checkpoint 的风险。当前改为图状态只保存随机隔离的 `contextKey`，`TravelPlanningContextRegistry`以并发 Map 承载一次同步执行的框架无关上下文，并在 `finally`无条件清理。该设计已由真实图条件边测试验证，不靠 mock 绕过图执行。

注册表是单 JVM、单次同步图执行的技术桥接，不是持久化 checkpoint。它适合当前单机约束和“持久任务领取后一次执行”的代码阶段；进程崩溃时不能从图中间节点恢复内存上下文，仍由 MySQL run、消息、记忆与 Outbox 重新判断任务状态，已经向收费模型发出的未知结果不得盲目重放。若未来需要多实例迁移或节点级恢复，应把最小、无敏感数据、版本化的节点状态显式持久化，不能把整个业务对象序列化进框架。

高德当前覆盖天气和路线，不能证明景区临时闭园、实时库存或门票政策；知识库证据不可用时也不得生成伪引用。草稿中未核实的开放时间、价格和政策必须携带复核提示，领域校验会拒绝“未核实且无提示”的项目。图目前每次执行编译一次，优先保证节点依赖和条件边清晰；若后续性能测试证明有成本，再在保持无共享可变状态的前提下缓存已编译图。

### 13.4 当前结论

本轮反例证明三路职责划分、框架隔离和关键条件边可运行，42 项 JUnit、Spring 无凭据离线装配及 356 类/906 方法/405 流程块业务质量门禁通过。结论仅覆盖本地代码与确定性规则；真实百炼结构化输出、流式断线、真实高德/知识库数据、Milvus 召回质量、多设备体验和进程崩溃恢复仍待后续阶段。按照用户决定，本节是 hello-travel 项目方案，不在本轮转写为公共 DDD+AI 规范。

## 14. 1.1调用链代码完成度再推演

SRC-058要求先完成代码，再由用户沿调用链推演。本轮没有改变“Controller持久化任务、Outbox异步投递、TravelApplication三路判断、Planning Graph业务编排”的架构，重点修复实现完成度。

字符串`DIALOGUE / FACT_QUERY / PLANNING`已收敛为`TravelIntentMode`；模型调用阶段收敛为跨对话能力共用的`ChatModelStage`；Graph注册与持久化检查点统一使用`TravelPlanningNode`。`TravelAiContract`集中保存图版本与提示契约版本，当前接受的新任务使用`travel-planning-v2 / travel-ai-v2`，避免Domain、Application和OutAdaptor各自维护同义硬编码。提示正文仍明确告诉模型枚举允许值，这是结构化输出约束，不是调用链动作分发。

Application新链路不再调用会丢失分类的通用`Result.require()`，而是使用`ApplicationFailures.required`保留下层稳定错误码。模型返回成功但无结构化数据时登记失败调用并映射为`UNAVAILABLE`；Graph节点捕获已分类`ApplicationException`后原样传播，不能再次覆盖为通用失败。`TravelApplication`将同一错误码写入已领取run终态并返回调用方，预算超限反例已验证为`CONTEXT_LIMIT`。

`TravelPlanDomainService`在原时间重叠、预算和未核实提示之外，新增逐日升序与起止日期完整覆盖、相邻活动交通方式/分钟及间隔、负费用、总额不得小于已知明细、已核实事实必须带来源、未核实活动必须汇总到待核实清单。确定性渲染同步展示来源、活动费用和总费用，不由模型在校验后再次改写。

高德转换不再把`forecasts`原始JSON直接放入上下文：只保留城市、发布时间、最多七天的昼夜天气与温度；路线距离和时长必须为正数并转换为公里/分钟。无天气/路线请求时跳过外部调用；供应商字段缺失继续走显式降级，不能把零值包装为可用事实。

新增三组调用链证据：`TravelApplicationRoutingTest`覆盖三个意图分支和错误分类；`TravelPlanningGraphTest`覆盖完整成功、一次修订成功、两次修订后停止及领域反例；`TravelFactOutputConverterTest`覆盖外部响应裁剪与非法事实拒绝。当前52项JUnit通过，Spring离线装配通过，业务门禁扫描360类、916方法/构造器、413流程块，违规0。真实供应商和基础设施联调仍未执行，本轮结论只说明代码路径和本地反例成立。

## 15. 动作级应用服务的实现边界（SRC-062）

认证、对话和知识库都以“一个业务动作对应一个AppService”为实现单位。入口AppService仅依据受控动作标识选择对应实现，处理公共的边界异常和`Result`封装；动作AppService直接完成自己的业务流程，不再通过`*ActionOperations`转发。

因此登录、刷新、取消运行、提交消息、上传文档等流程能从类型名定位到唯一实现。跨动作复用仅限于认证主体校验、受控对象读取和运行快照组装，放在各业务域私有Support中；Support不含动作选择，不构成业务流程聚合。该选择降低定位成本，也避免`usecase`目录成为与业务域平行的第二层组织维度。

## 16. 调度与SSE输入适配边界（SRC-063）

`@Scheduled` 是Spring时间触发协议，属于输入适配器。认证保留期清理由 `adaptor.auth.input.scheduler.RetentionScheduler` 触发；对话域的持久任务、运行恢复和知识索引对账由 `adaptor.chat.input.scheduler.BackgroundJobScheduler` 触发；SSE轮询由 `adaptor.chat.input.scheduler.SyncStreamScheduler` 触发。Scheduler只负责时间触发、异常记录和调用Application，不读取Repository、不执行业务状态变更。

Application 中的 `OutboxDispatchAppService` 与 `RetentionAppService` 是可调用的处理过程，已不带调度注解。`SyncStreamController` 只建立HTTP SSE订阅；`SyncStreamPublisher` 管理连接状态和有界发送；Scheduler独立触发发布。项目质量门禁拒绝在非 adaptor scheduler 包声明 `@Scheduled`，从源码层防止回退。

### 12.14 认证模型修订：验证码统一登录与首次自动建号

本轮根据用户已确认的认证意图修订此前“先注册、后验证码登录”的设计。此前的独立注册动作、`REGISTER`验证码用途和“仅已有账号可申请登录验证码”会让首次用户收到有效验证码后仍无法进入系统，属于产品流与技术实现共同偏差。

新的认证契约如下：

1. `LOGIN`验证码不以账号是否存在作为发码或投递条件；有效验证码是邮箱归属证明。
2. 密码登录要求账号存在且已配置密码；账号不存在、未配置密码或密码错误统一为认证失败，避免密码入口泄露账号状态。
3. 验证码登录在一个短事务内读取账号；不存在时受唯一邮箱约束创建邮箱已验证账号，随后消费验证码、为当前浏览器替换会话、写同步事件和发件箱。任何一步失败整体回滚，验证码不被错误消费。
4. `password_hash`允许为空，表达“该账号尚未设置密码”，不以服务端随机未知密码伪造已设置状态。`RESET_PASSWORD`用途用于设置首个密码或重置已有密码，成功后提高认证代次并撤销旧会话。
5. 前端不再让用户选择“注册”；邮箱验证码登录覆盖首次和既有用户。密码设置/重置保留为独立显式动作。

反例推演：两个相同首次登录请求都在事务前验证同一验证码时，唯一邮箱与验证码CAS使至多一个事务创建账号并消费验证码；另一个事务读取已消费挑战后安全失败。首次登录与会话签发之间不产生无会话的半账号；若唯一账号已由另一事务创建，当前事务读取既有账号后仍须成功消费自己的有效挑战才签发会话。密码为空不会交给密码校验器，密码入口统一使用等成本虚拟摘要并返回认证失败。
