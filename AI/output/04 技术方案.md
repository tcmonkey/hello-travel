...
模型预算由ModelContextPolicy统一，配置键travel.model.context-window / output-reserve / safety-reserve / compression-input-limit默认32768/4096/4096/20000。配置非法在初始化拒绝；辅助输出最大2048且受配置output-reserve约束，回答输出使用配置预留；所有非嵌入模型输入在IO前估算容量并拒绝超预算。前端只展示服务端预算，缺失视为未知。KnowledgeIndexPolicy统一text-embedding-v4、1024和hello_travel_kb_v1_d1024；Domain通过EmbeddingProfileValue接受已选策略；MySQL元数据和Milvus向量职责不混淆。
