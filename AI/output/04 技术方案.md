...
|---|---|---|---|
| 认证auth | domain/auth/service/AuthDomainService | UserAccount、Device、LoginSession、EmailChallenge、RefreshReceipt | AuthWriteAppService / AuthWriteAppAssembler / 调用者直接注入所需Repository |
| 对话chat | domain/chat/service/ChatDomainService | Conversation、Message、ChatRun、ModelInvocation | ChatWrites / ChatWriteAppAssembler / 调用者直接注入所需Repository |
| 记忆memory | domain/memory/service/MemoryDomainService | MemorySummary、MemoryFact、MemoryFactSource | MemoryWriteAppService / MemoryWriteAppAssembler / 调用者直接注入所需Repository |
| 知识库knowledge | domain/knowledge/service/KnowledgeDomainService | KnowledgeDocument、KnowledgeChunk、IndexJob | KnowledgeWriteAppService / KnowledgeWriteAppAssembler / 调用者直接注入所需Repository |
| 同步sync | domain/sync/service/SyncDomainService | SyncEvent、OutboxEvent | SyncWrites / SyncWriteAppAssembler / 调用者直接注入所需Repository |
...
领域服务名称表达业务职责，不用`Write`后缀重复用例模式；保存、删除等公开动作和对应Param保留写入语义。`AuthWriteAppService`等应用协作对象属于写入用例的实现辅助，未被当作领域服务身份。Application读取不再通过`AuthRepositories`、`ChatRepositories`、`MemoryRepositories`或`SyncRepositories`聚合，调用者直接声明实际仓储依赖；构造器依赖数量因此反映真实职责规模。参考工程的`DddWriteDomainService`仅用于教学路径标记，不作为本项目命名模板。

普通读仍由本业务Application/Flow访问相应仓储并经assembler转换，外部读仍经对应OutAdaptor；不为形式凑五个DomainService。PrivacyCleanup/MemoryFlow显式调用memory和chat协作；后台保留策略显式调用auth和sync协作；跨域控制在应用层，不将其他域仓储注入DomainService。Transactions仍以认证账号版本控制提交序列；SyncEvents只注入SyncWrites，删除原本未使用的仓储依赖，无循环装配。
