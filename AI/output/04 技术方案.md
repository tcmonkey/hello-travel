...
领域服务名称表达业务职责，不用`Write`后缀重复用例模式；保存、删除等领域方法和Param保留具体动作语义。Application不再设置`*Writes`或`*WriteAppService`作为通用持久化中转，注册、登录、会话、消息、记忆、知识索引与同步发布等业务流程通过各自`DomainParamAssembler`转换完整聚合后直接调用所属DomainService。Application读取也不通过`*Repositories`聚合；构造器依赖据此暴露真实职责规模。
