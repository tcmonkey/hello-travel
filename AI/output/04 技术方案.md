...
mode: "full"
updated: "2026-09-19"
sources: [SRC-001, SRC-002, SRC-003, SRC-004, SRC-005, SRC-006, SRC-007, SRC-008, SRC-009, SRC-010, SRC-011, SRC-012, SRC-013, SRC-014, SRC-015, SRC-016, SRC-017, SRC-018, SRC-019, SRC-020, SRC-021, SRC-022, SRC-023, SRC-024, SRC-025, SRC-026, SRC-030, SRC-031, SRC-050, SRC-052, SRC-053, SRC-054, SRC-055, SRC-056, SRC-057, SRC-058]
---
...
|---|---|---|---|
| 认证auth | domain/auth/service/AuthDomainService | UserAccount、Device、LoginSession、EmailChallenge、RefreshReceipt | AuthWrites / AuthWriteApplicationAssembler / AuthRepositories |
| 对话chat | domain/chat/service/ChatDomainService | Conversation、Message、ChatRun、ModelInvocation | ChatWrites / ChatWriteApplicationAssembler / ChatRepositories |
| 记忆memory | domain/memory/service/MemoryDomainService | MemorySummary、MemoryFact、MemoryFactSource | MemoryWrites / MemoryWriteApplicationAssembler / MemoryRepositories |
| 知识库knowledge | domain/knowledge/service/KnowledgeDomainService | KnowledgeDocument、KnowledgeChunk、IndexJob | KnowledgeWrites / KnowledgeWriteApplicationAssembler / KnowledgeRepositories |
| 同步sync | domain/sync/service/SyncDomainService | SyncEvent、OutboxEvent | SyncWrites / SyncWriteApplicationAssembler / SyncRepositories |
...
领域服务名称表达业务职责，不用`Write`后缀重复用例模式；保存、删除等公开动作和对应Param保留写入语义。`AuthWrites`等应用协作对象属于写入用例的实现辅助，未被当作领域服务身份。参考工程的`DddWriteDomainService`仅用于教学路径标记，不作为本项目命名模板。
...
SRC-052进一步指出上一轮仍在application/adaptor一级按HTTP、model、file、mail等技术概念分包。当前改为认证、对话、知识库三条产品主线；同步、记忆与旅行规划是对话内部能力，邮件与安全归认证，文件、向量和嵌入归知识库，跨业务Web协议支撑独立放在`adaptor.web.support`。一级`adaptor/http`、`application/model`和`adaptor/model`均已删除。

原通用模型命令依赖`action + 多组可空字段`，把意图、摘要、事实提取、回答和嵌入组合成一个端口，新增能力容易继续增加分支。本轮按照业务目的拆成`TravelIntentAgent`、`TravelPlanningAgent`、`MemoryAgent`和`KnowledgeEmbeddingAgent`；提示规则和固定参数由所属assembler承载，Application不再识别模型厂商或动态动作。共享结果也按对话和知识嵌入分开，避免一个DO同时携带文本与向量两种互斥形态。

对话侧LangChain4j接口使用显式`@AiService`；start提供命名`ChatModel`和`StreamingChatModel`，回答接口返回Flux并由适配器累积完整草稿、限制长度、执行租约进度回调。知识侧start提供命名`EmbeddingModel`，索引与查询复用同一模型实例。无密钥时全部装配禁用模型，保证离线启动不触发外部调用。
...
每条业务输入边界固定使用同级`input.controller`与`input.assembler`子包。Controller承载HTTP入口、边界捕获和调用编排；Assembler承载Request/path/query与Command、Application Result与Response之间的协议转换。两者不再平铺于`input`根包，SSE入口同样属于对话业务的controller包，不因传输方式另立业务维度。

认证、对话和知识库输入代码均已迁移；DDD参考Controller及随包快照同步。`QUALITY-PACKAGE`在Java语法扫描阶段拒绝错放的`*Controller`和`*Assembler`，完整构建证明Spring扫描、测试引用和HTTP映射未因包迁移失效。
