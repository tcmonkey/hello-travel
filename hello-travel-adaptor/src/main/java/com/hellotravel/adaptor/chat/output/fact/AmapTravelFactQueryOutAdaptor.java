package com.hellotravel.adaptor.chat.output.fact;

import com.fasterxml.jackson.databind.JsonNode;
import com.hellotravel.adaptor.exception.AdaptorErrorCode;
import com.hellotravel.adaptor.chat.output.fact.converter.TravelFactOutputConverter;
import com.hellotravel.application.support.Json;
import com.hellotravel.application.chat.travel.fact.adaptor.TravelFactQueryAdaptor;
 ...
 */
@Component
public final class AmapTravelFactQueryOutAdaptor implements TravelFactQueryAdaptor {

    private final Environment environment;
    private final TravelFactOutputConverter travelOutputConverter;

    /**
     * 复用连接池和超时配置，避免每个事实子请求重新创建客户端。
     *
     * @author AIGenerator
     */
    ...
    public AmapTravelFactQueryOutAdaptor(
            Environment environment, TravelFactOutputConverter travelOutputConverter) {
     ...
     *
     * @author AIGenerator
     * @param travelFactQueryCommand 当前用例命令，归属来自服务端
     * @return 当前操作的业务结果
     */
    @Override
    public Result<TravelDO> query(TravelFactQueryCommand travelFactQueryCommand) {
        try {
            ...
            try {
                ...
                if (travelFactQueryCommand.weather() && valid(travelFactQueryCommand.city())) {
                    var point = geo(key, travelFactQueryCommand.city());
                    var weather =
                            get(
                ...
                if (travelFactQueryCommand.route()
                        && valid(travelFactQueryCommand.origin())
                        && valid(travelFactQueryCommand.destination())) {
                    var origin = geo(key, travelFactQueryCommand.origin());
                    var destination = geo(key, travelFactQueryCommand.destination());
                    var route =
                            get(
