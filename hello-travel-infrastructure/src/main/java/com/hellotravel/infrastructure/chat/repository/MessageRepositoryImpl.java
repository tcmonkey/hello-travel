...
import com.hellotravel.infrastructure.chat.converter.MessagePersistenceConverter;
...
public class MessageRepositoryImpl extends TravelBaseRepository<MessageMapper, MessagePO>
    ...
    private final MessagePersistenceConverter messagePersistenceConverter;
    ...
    public MessageAggregate findById(Long id) {
        ...
        return po == null ? null : messagePersistenceConverter.restore(po);
    }
    ...
    public List<MessageAggregate> query(QueryValue queryValue) {
        ...
        return page(page, wrapper).getRecords().stream()
                .map(messagePersistenceConverter::restore)
                .toList();
    }
    ...
    public Boolean save(MessageAggregate aggregate) {
        try {
            // 1. 转换完整聚合为本仓储PO，映射与状态决策分开。
            MessagePO po = messagePersistenceConverter.toPersistence(aggregate);
    ...
    public MessageRepositoryImpl(MessagePersistenceConverter messagePersistenceConverter) {
        this.messagePersistenceConverter = messagePersistenceConverter;
    }
}
