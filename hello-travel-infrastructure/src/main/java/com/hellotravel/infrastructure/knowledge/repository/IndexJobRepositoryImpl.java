...
/**
 * ht_index_job仓储，标准安全条件构造器与显式版本CAS，不使用自定义SQL。
 *
 * @author AIGenerator
 */
@Repository
public class IndexJobRepositoryImpl extends TravelBaseRepository<IndexJobMapper, IndexJobPO>
     ...
     * @return 当前操作的业务结果
     */
    public IndexJobAggregate findById(Long id) {
     ...
     * @return 当前操作的业务结果
     */
    public List<IndexJobAggregate> query(QueryValue queryValue) {
        // 1. 按字段白名单组装参数绑定条件，禁止任意列或拼接SQL。
        QueryWrapper<IndexJobPO> wrapper =
                conditions(
                        queryValue,
                        Set.of(
                                "id",
