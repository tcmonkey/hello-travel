package com.hellotravel.application.knowledge.job;
 ...
 */
@Component
public final class KnowledgeIndexJob {
    ...
    public void execute(Long jobId) {
        ...
        // 3. 按可信内部标识读取知识文档当前快照。
        var document = knowledgeRepositories.knowledgeDocument.findById(job.documentId()).entity();
        ...
        try {
            ...
            String owner = authRepositories.userAccount.findById(job.userId()).entity().publicId();
    ...
    private void current(IndexJobEntity expected) {
        // 1. 按可信内部标识读取索引任务当前快照。
        var job = knowledgeRepositories.indexJob.findById(expected.id()).entity();
        var doc = knowledgeRepositories.knowledgeDocument.findById(job.documentId()).entity();
    ...
    private void finish(IndexJobEntity expected, boolean success, String error) {
        transactions.mutate(
                ...
                account -> {
                    // 1. 按可信内部标识读取索引任务当前快照。
                    var old = knowledgeRepositories.indexJob.findById(expected.id()).entity();
                    var doc =
                            knowledgeRepositories
                                    .knowledgeDocument
                    ...
                    if ("RUNNING".equals(old.status())
                        ...
                        Transactions.require(
                                knowledgeWrites.saveIndexJob(new IndexJobAggregate(done)));
                        if (same
    ...
    public void reconcile() {
        ...
        for (var row : documents) {
            ...
            var doc = row.entity();
            String owner = authRepositories.userAccount.findById(doc.userId()).entity().publicId();
                ...
                transactions.mutate(
                        ...
                        account -> {
                            ...
                            for (var chunk :
                                ...
                                Transactions.require(
                                        knowledgeWrites.saveKnowledgeChunk(
    ...
    private IndexJobEntity claim(Long jobId) {
        ...
        IndexJobEntity job =
                transactions.plain(
                        () -> {
                            // 1. 按可信内部标识读取索引任务当前快照。
                            var stored = knowledgeRepositories.indexJob.findById(jobId);
                            ...
                            Transactions.require(
                                    knowledgeWrites.saveIndexJob(new IndexJobAggregate(next)));
                            // 5. 提供本事务或回调的处理结果，完成责任由所属外层流程承接。
                            return knowledgeRepositories.indexJob.findById(jobId).entity();
                        });
