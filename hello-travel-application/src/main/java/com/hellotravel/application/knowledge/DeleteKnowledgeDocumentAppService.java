 ...
 */
@Component
public final class DeleteKnowledgeDocumentAppService extends KnowledgeDocumentSupport
   ...
   * @author AIGenerator
   */
  public DeleteKnowledgeDocumentAppService(
  ...
  public KnowledgeAppResult execute(KnowledgeCommand command) {
    // 1. 固定当前用例为删除，避免共用分支根据外部动作字段改变行为。
    boolean retry = false;
    // 2. 在短事务内处理删除，归属和版本必须在事务内复核。
