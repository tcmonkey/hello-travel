 ...
 */
@Component
public final class RetentionAppService {
  ...
  public void expireCredentials() {
          ...
          for (var row :
              emailChallengeRepository.query(
                  QueryValue.all("id", 100)
                      .where("status", "IN", java.util.List.of("PENDING_SEND", "ISSUED"))
                      .where("expires_at", "LT", now()))) {
            Transactions.require(
                ApplicationFailures.required(
                        authDomainService.saveEmailChallenge(
                            authDomainParamAssembler.emailChallenge(
                                new EmailChallengeAggregate(row.entity()).delivery("EXPIRED"))))
                    .saved());
          }
    ...
    for (var row :
      ...
      if (!"ACTIVE"
          .equals(userAccountRepository.findById(row.entity().userId()).entity().status())) {
        continue;
      }
      transactions.mutate(
          row.entity().userId(),
          account -> {
            // 1. 按可信内部标识读取登录会话当前快照。
            var current = loginSessionRepository.findById(row.entity().id()).entity();
            // 2. 依据实体当前状态与允许的操作处理分支，避免继续使用无效数据。
            if ("ACTIVE".equals(current.status()) && current.refreshExpiresAt().isBefore(now())) {
              Transactions.require(
                  ApplicationFailures.required(
