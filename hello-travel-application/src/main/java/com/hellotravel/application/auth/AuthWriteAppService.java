 ...
 */
@Component
public final class AuthWriteAppService {

    private final AuthDomainService service;
    private final AuthWriteAppAssembler authWriteAppAssembler;

    public AuthWriteAppService(
            AuthDomainService service,
            AuthWriteAppAssembler authWriteAppAssembler) {
        this.service = service;
        this.authWriteAppAssembler = authWriteAppAssembler;
    }
    ...
    public Boolean saveUserAccount(UserAccountAggregate aggregate) {
        ...
        var param = authWriteAppAssembler.write(aggregate);
    ...
    public Boolean removeRefreshReceipt(Long id) {
        // 1. 用应用assembler绑定完整聚合或删除标识，禁止在调用处拼装Param。
        var param = authWriteAppAssembler.removeRefreshReceipt(id);
        // 2. 执行领域入口，状态、并发和不可变归属由领域负责。
