 ...
 * @author AIGenerator
 */
abstract class BaseAuthActionAppService implements AuthActionAppInterface {
    ...
    BaseAuthActionAppService(String action, AuthActionOperations operations) {
        this.action = action;
     ...
     */
    @Override
    public final AuthAppResult execute(AuthCommand authCommand) {
        return executeAction(authCommand, operations);
    }

    abstract AuthAppResult executeAction(AuthCommand authCommand, AuthActionOperations operations);
}
