 ...
 */
@Component
public final class CancelChatRunAppService extends ChatConversationSupport
   ...
   * @author AIGenerator
   */
  public CancelChatRunAppService(
          ...
          if (List.of("ACCEPTED", "RUNNING").contains(r.status())) {
            Transactions.require(
                ApplicationFailures.required(
                    ...
                    .saved());
            MessageEntity m = messageRepository.findById(r.assistantMessageId()).entity();
            Transactions.require(
                ApplicationFailures.required(
                        chatDomainService.saveMessage(
                            chatDomainParamAssembler.message(
                                new MessageAggregate(m)
                                    .progress(m.content(), "CANCELLED", m.citationsJson()))))
                    .saved());
          }
          // 3. 同事务记录用户提交序号与同步事件，推送不能代替持久化。
          events.append(account, "run.cancelled", r.publicId(), r.version() + 1, null, "{}");
