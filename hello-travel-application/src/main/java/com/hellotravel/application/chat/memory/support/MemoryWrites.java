package com.hellotravel.application.chat.memory.support;

import com.hellotravel.application.chat.memory.assembler.MemoryWriteApplicationAssembler;
 ...
 */
@Component
public final class MemoryWrites {

    private final MemoryDomainService service;
    private final MemoryWriteApplicationAssembler memoryWriteApplicationAssembler;

    public MemoryWrites(
            MemoryDomainService service,
            MemoryWriteApplicationAssembler memoryWriteApplicationAssembler) {
        this.service = service;
        this.memoryWriteApplicationAssembler = memoryWriteApplicationAssembler;
    }
    ...
    public Boolean saveMemorySummary(MemorySummaryAggregate aggregate) {
        ...
        var param = memoryWriteApplicationAssembler.write(aggregate);
    ...
    public Boolean removeMemoryFactSource(Long id) {
        // 1. 用应用assembler绑定完整聚合或删除标识，禁止在调用处拼装Param。
        var param = memoryWriteApplicationAssembler.removeMemoryFactSource(id);
        // 2. 执行领域入口，状态、并发和不可变归属由领域负责。
