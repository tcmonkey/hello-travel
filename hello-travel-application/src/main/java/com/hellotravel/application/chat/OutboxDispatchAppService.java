 ...
 */
@Component
public final class OutboxDispatchAppService {
  ...
  public OutboxDispatchAppService(
  ...
  public void dispatch() {
      ...
      var claimed =
          transactions.plain(
              () -> {
                ...
                var old = outboxEventRepository.findById(row.entity().id()).entity();
                ...
                var next =
                    new OutboxEventAggregate(old)
                        .claimed(
                            old.attemptCount() + 1, old.leaseFence() + 1, now().plusMinutes(15))
                        .entity();
                // 4. 持久化当前完整聚合，失败必须中断事务而非继续提交。
                Transactions.require(
                    ApplicationFailures.required(
                            syncDomainService.saveOutboxEvent(
                                syncDomainParamAssembler.outboxEvent(
                                    new OutboxEventAggregate(next))))
                        .saved());
    ...
    for (var row : indexJob.query(QueryValue.all("id", 2).where("status", "EQ", "PENDING"))) {
      try {
  ...
  private void email(String id) {
    ...
    var rows = emailChallengeRepository.query(QueryValue.all("id", 1).where("public_id", "EQ", id));
    // 2. 没有待处理任务时结束当前领取窗口。
    if (rows.isEmpty()) {
      return;
    }
    ...
    String encrypted = java.util.Base64.getEncoder().encodeToString(challenge.deliveryCiphertext());
  ...
  private void complete(OutboxEventEntity expected, boolean success, String error) {
    transactions.plain(
        () -> {
          ...
          if ("DELIVERING".equals(old.status()) && old.leaseFence().equals(expected.leaseFence())) {
            boolean retrySafe =
                !success
                    && java.util.Set.of("SYNC", "MEMORY_REBUILD").contains(old.eventType())
                    && old.attemptCount() < old.maxAttempts();
            var next =
                old.completed(
                    success ? "DELIVERED" : retrySafe ? "PENDING" : "DEAD",
                    retrySafe
                        ? now().plusSeconds(Math.min(300, 1L << Math.min(8, old.attemptCount())))
                        : old.nextAttemptAt(),
                    success ? now() : null,
                    error);
            Transactions.require(
