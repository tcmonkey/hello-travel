package com.hellotravel.application.chat.travel.fact.evidence;
...
import com.hellotravel.application.chat.travel.fact.assembler.TravelFactQueryAssembler;
 ...
 */
@Component
public final class TravelEvidenceCollector {

    private final TravelFactQueryAdaptor travelFactQueryAdaptor;
    private final TravelFactQueryAssembler travelFactQueryAssembler;
    private final RagRetrievalService ragRetrievalService;

    public TravelEvidenceCollector(
            TravelFactQueryAdaptor travelFactQueryAdaptor,
            TravelFactQueryAssembler travelFactQueryAssembler,
            RagRetrievalService ragRetrievalService) {
        this.travelFactQueryAdaptor = travelFactQueryAdaptor;
        this.travelFactQueryAssembler = travelFactQueryAssembler;
    ...
    public Result<Boolean> collectDestinationEvidence(TravelConversationContext context) {
        try {
            ...
            if (context.intent().knowledge()
                ...
                context.sources(ragRetrievalService.retrieve(context.run().userId(), context.input()));
            } else {
    ...
    public Result<Boolean> collectRealtimeFacts(TravelConversationContext context) {
        try {
            ...
            var command = travelFactQueryAssembler.command(context.intent());
