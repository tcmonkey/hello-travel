 ...
 */
@Component
public final class MemoryContextAppService {
    ...
    public MemoryContextAppService(
        ...
        this.memoryAgentAppAssembler = memoryAgentAppAssembler;
    }
    ...
    public List<MessageEntity> recentEntities(ChatRunEntity run) {
        // 1. 按可信内部标识读取消息当前快照。
        var input = messageRepository.findById(run.userMessageId()).entity();
        var rows =
    ...
    public String facts(ChatRunEntity run) {
        ...
        for (var row :
            ...
            for (var source :
                ...
                var original = messageRepository.findById(source.entity().messageId());
                if (original != null
    ...
    public String compress(
        ...
        var result = memoryAgent.summarize(memoryAgentAppAssembler.summary(input));
        ...
        var inputEntity = messageRepository.findById(run.userMessageId()).entity();
    ...
    public void extract(String publicRunId) {
        ...
        var prior =
                modelInvocationRepository.query(
                        QueryValue.all("id", 1)
        ...
        var output = memoryAgent.extract(memoryAgentAppAssembler.extraction(input));
        ...
        var data =
                JsonUtil.read(output.data().text().replace("```json", "").replace("```", "").strip());
    ...
    private void persistFacts(
        ...
        transactions.mutate(
                ...
                account -> {
                    // 1. 按可信内部标识读取对话当前快照。
                    var current = conversationRepository.findById(c.id()).entity();
                    var source = messageRepository.findById(input.id()).entity();
