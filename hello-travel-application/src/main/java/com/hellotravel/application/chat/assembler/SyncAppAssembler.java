 ...
 */
@Component
public final class SyncAppAssembler {
     ...
     * @author AIGenerator
     */
    public SyncEventAppResult event(SyncEventEntity entity) {
        return new SyncEventAppResult(
                entity.eventSeq(),
                entity.eventType(),
     ...
     * @author AIGenerator
     */
    public SyncAppResult page(java.util.List<SyncEventAggregate> rows, long highWater) {
        // 1. 投影持久事件元数据，应用入口已经校验归属及连续性。
        var items = rows.stream().map(x -> event(x.entity())).toList();
        // 2. 依据最后事件序号判断是否继续补齐，保持原有空页语义。
        return new SyncAppResult(
                items,
                highWater,
