package com.hellotravel.application.sync.support;

import com.hellotravel.application.support.ApplicationFailures;
import com.hellotravel.application.sync.assembler.SyncWriteApplicationAssembler;
...
public final class SyncWrites {
    ...
    private final SyncWriteApplicationAssembler syncWriteApplicationAssembler;

    public SyncWrites(
            SyncDomainService service,
            SyncWriteApplicationAssembler syncWriteApplicationAssembler) {
        this.service = service;
        this.syncWriteApplicationAssembler = syncWriteApplicationAssembler;
    }
    ...
    public Boolean saveSyncEvent(SyncEventAggregate aggregate) {
        ...
        var param = syncWriteApplicationAssembler.write(aggregate);
    ...
    public Boolean removeSyncEvent(Long id) {
        // 1. 用应用assembler绑定完整聚合或删除标识，禁止在调用处拼装Param。
        var param = syncWriteApplicationAssembler.removeSyncEvent(id);
        // 2. 执行领域入口，状态、并发和不可变归属由领域负责。
