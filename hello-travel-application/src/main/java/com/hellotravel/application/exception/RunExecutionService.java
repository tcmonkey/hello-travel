...
public final class RunExecutionService {
        ...
        return transactions.mutate(
                ...
                account -> {
                    ...
                    ChatRunEntity current =
                            chatRunRepository.findById(selected.id()).entity();
                    ...
                    return chatRunRepository.findById(current.id()).entity();
                });
    }
    ...
    public ChatRunEntity requireCurrent(ChatRunEntity expected) {
        // 1. 按可信内部标识读取生成任务当前快照。
        ChatRunEntity current = chatRunRepository.findById(expected.id()).entity();
        ConversationEntity c =
                conversationRepository.findById(expected.conversationId()).entity();
    ...
    public void invocationComplete(
            String id, ChatModelDO response, long latency, boolean success) {
        transactions.plain(
                () -> {
        ...
        transactions.mutate(
                ...
                account -> {
                    ...
                    events.append(
                            ...
                            output.version() + 1,
                            null,
                            JsonUtil.encode(Map.of("runId", expected.publicId())));
    ...
    public void fail(ChatRunEntity expected, String error) {
        transactions.mutate(
                ...
                account -> {
                    // 1. 按可信内部标识读取生成任务当前快照。
                    ChatRunEntity current =
                            chatRunRepository.findById(expected.id()).entity();
                    // 2. 核对租约持有者、栅栏和到期时间，旧执行者不能提交。
        ...
        for (var row :
            ...
            transactions.mutate(
                    ...
                    account -> {
                        // 1. 按可信内部标识读取生成任务当前快照。
                        ChatRunEntity current =
                                chatRunRepository.findById(row.entity().id()).entity();
                        // 2. 核对租约持有者、栅栏和到期时间，旧执行者不能提交。
                        if ("RUNNING".equals(current.status())
                                && current.leaseUntil().isBefore(now())) {
                            Transactions.require(
    ...
    public void recoverAccepted() {
        for (var row :
            ...
            transactions.mutate(
                    ...
                    account -> {
                        // 1. 按可信内部标识读取生成任务当前快照。
                        var current = chatRunRepository.findById(selected.id()).entity();
