package com.hellotravel.application.memory.cleanup;
...
import com.hellotravel.application.memory.support.MemoryRepositories;
import com.hellotravel.application.memory.support.MemoryWrites;
 ...
 */
@Component
public final class PrivacyCleanupService {
    ...
    public void execute(Long conversationId) {
        // 1. 按可信内部标识读取对话当前快照。
        var stored = chatRepositories.conversation.findById(conversationId);
        ...
        transactions.mutate(
                ...
                account -> {
                    // 1. 按可信内部标识读取对话当前快照。
                    var c = chatRepositories.conversation.findById(conversationId).entity();
                    ...
                    for (var row :
                        ...
                        Transactions.require(
                                memoryWrites.saveMemorySummary(new MemorySummaryAggregate(next)));
                    }
                    ...
                    for (var row :
                        ...
                        for (var source :
                            ...
                            Transactions.require(
                                    memoryWrites.removeMemoryFactSource(source.entity().id()));
                        }
                        var next = old.redacted();
                        Transactions.require(
                                memoryWrites.saveMemoryFact(new MemoryFactAggregate(next)));
                    }
